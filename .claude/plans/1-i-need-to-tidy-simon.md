...
### 4. KNMI WBGT import ("latest heat force")  ☐
...
| B3 | `mainMap.html:526-527` | `Housing` entry in `ADMIN_PANEL_TOOLS` omits `level` → URL matches nothing in `housing/urls.py` after year/location change | 2, 3 | ☐ |
...
| B12 | `administrative/models.py:72-76` (`City.save`) | Overwrites directly-imported `currentPopulation` with District sum (0 if none) – conflicts with CBS import | 1, 2 | ☐ decide |
...
| B19 | `importer/views_external.py:33,46` | New `source` without `SOURCE_INFO` entry → `KeyError`; `source_order` hardcoded | 4 | ☐ (handle when adding KNMI) |
...
2. **WBGT heat-stress thresholds** for `classify_wbgt` (typical bands ~25/28/30/33 °C) – confirm the bands before hard-coding.
3. **WBGT file format / GDAL driver** – verify with one real downloaded file.
...
### Phase 3 – Workstream 2: population curve (CBS 85173NED)  ☐
...
### Phase 4 – Workstream 4: KNMI WBGT import  ☐
