...
The same cascade infrastructure was later extended to a second field, `urban_area`, by generalizing `_recompute_population()` to sum both `currentPopulation` and `urban_area` in one aggregate rather than duplicating the cascade logic. Unlike population, though, `urban_area` has no independent source of truth at the leaf: it is itself *derived*, computed from the spatial overlap between each Neighbourhood's boundary and land-cover survey polygons (`physicalEnv.LandCoverVector`) whose classification indicates built-up or transport surfaces, restricted to the most recent survey year available for that city so repeat surveys aren't double-counted. This chains two independent signal-driven systems: a `physicalEnv` signal recomputes each Neighbourhood's `urban_area` whenever land-cover data changes (using PostGIS's intersection-area operator, `ST_Intersection`), and that write manually re-enters the `administrative` cascade afterward, since `QuerySet.update()` deliberately does not raise `post_save` to trigger it automatically.
...
| Google Earth Engine | Raster | Global surface water, forest cover, CHIRPS precipitation, HRSL population density |
